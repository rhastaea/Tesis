...
# 2? subo la tabla .csv que tiene la informaci?n de cada estaci?n de muestreo
#(antes de subirla chequear que est?n bien fecha/hora y corregir)

datastation <- read.csv("datastation_final.csv",
                        header = TRUE, 
                        sep = ",") 
# ver si el separados es coma o punto y coma, y modificar en sep=""
...
exiftooldir <- "C:/Users/shalo/OneDrive/Escritorio/Ilán/Tesis/Archivos ahora"  #Justo lo puse en el mismo directorio
...
datacam <- recordTable(inDir = "M_todas",
                       ...
                       writecsv = F) #Acá le puedo poner T para que me descargue el .CSV
datacam$DateTimeOriginal <- as.factor(datacam$DateTimeOriginal)

############# CERRAR ################ CREAR DATACAM_DELTA - TABLA DE IMÁGENES/ESPECIES ######

datacam_delta1 <- recordTable(inDir = "D:/TESIS/Archivos_R/M_todas",
                      IDfrom = "directory",
                      outDir = "D:/TESIS/Archivos_R",
                      minDeltaTime = 1,  # datos independientes definidos cada 60 min. POR DEFINIR
...
##################################### LEER DATACAM1 - TABLA DE IMÁGENES/ESPECIES ######
...
datacam_delta30 <- read.csv("datacam_delta30.csv", 
                             header = T,
                             sep = ",",
                             row.names = 1)
datacam_delta30$DateTimeOriginal <- as.factor(datacam_delta30$DateTimeOriginal)


datacam_delta60 <- read.csv("datacam_delta60.csv", 
                            header = T,
                            sep = ",",
                            row.names = 1)
datacam_delta60$DateTimeOriginal <- as.factor(datacam_delta60$DateTimeOriginal)

##################################### LEER DATACAM_MUESTREO PARA FILTRAR ####
...
##################################### PNGs para histogramas ####
sun <- readPNG("sun.png")
moon <- readPNG("moon.png")

rhea <- readPNG("rhea.png")
hydrochoerus <- readPNG("hydrochoerus.png")
sus <- readPNG("sus.png")
cingulata <- readPNG("cingulata.png")
mazama <- readPNG("mazama.png")
axis <- readPNG("axis.png")
canidae <- readPNG("canidae.png")
leopardus <- readPNG("leopardus.png")

##################################### HISTOGRAMA RADIAL!! ####
registers <- datacam_delta60 #Acá el script original llama "registers" a nuestro datacam. Decidí dejarlo así porque toca algunas cosas de la tabla y así no me modifica datacam.
...
ggdraw(plot_blanco) +
   draw_image(rhea, x=0.07, y = 0.88,
              width = 0.1, height = 0.1)


##################################### Histograma radial con superposición ##################

registers <- datacam_abril #ACÁ HAY QUE CAMBIAR EL DATACAM QUE QUIERO
...
(plot_blanco <- ggplot(especies, aes(x = decimal, fill = Species)) + 
      geom_histogram(breaks = seq(0, 24),
                     position = "identity", # Esto es para que me superponga las especies. 
                     colour = "black", 
